...
function collectWinnerRows(rootDir) {
  ...
  packets.forEach(({ packet }) => {
    ...
    ['weapons', 'hyungs', 'sparring'].forEach((key) => {
      ...
      placements.forEach((placement) => {
        ...
        const categoryRows = rowsByCategory.get(category) || new Map();
        const existing = categoryRows.get(`${divisionId}|${competitorId}`) || {
          name: String(competitor.name || competitorId),
          rank: String(competitor.rank || ''),
          school: String(competitor.studio || ''),
          divisionId,
          points: 0
        };

        existing.points += pointsForPlacement(placement.place);
        categoryRows.set(`${divisionId}|${competitorId}`, existing);
        rowsByCategory.set(category, categoryRows);
      });
    });
  });
...
function buildCategoryLines(category, rows) {
  ...
  const header = ['Name', 'Rank', 'School', 'Group Division Number', 'Total Points'];
  ...
  lines.push([
    ...
    header[2].padEnd(widths[2]),
    header[3].padEnd(widths[3]),
    header[4].padStart(widths[4])
  ].join(' | '));
  lines.push([
    '-'.repeat(widths[0]),
    '-'.repeat(widths[1]),
  ...
  rows.forEach((row) => {
    lines.push([
      ...
      row.rank.padEnd(widths[1]),
      row.school.padEnd(widths[2]),
      row.divisionId.padEnd(widths[3]),
      String(row.points).padStart(widths[4])
    ].join(' | '));
  });
  ...
  CATEGORY_ORDER.forEach((category) => {
    const rows = Array.from((rowsByCategory.get(category) || new Map()).values())
      ...
      .sort((left, right) => {
        ...
        if (left.divisionId !== right.divisionId) return left.divisionId.localeCompare(right.divisionId);
...
async function main() {
  ...
  const rowsByCategory = collectWinnerRows(inputDir);
  const lines = buildReportLines(rowsByCategory);
  const outputPath = path.join(inputDir, `Competitor-Winners-${buildTimestampForFile()}.pdf`);
  await writeTextPdf(outputPath, 'Competitor Winners', lines);
